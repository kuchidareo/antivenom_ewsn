from __future__ import annotations

from pathlib import Path
import argparse

import numpy as np
import pandas as pd


def _per_device_round(df: pd.DataFrame) -> pd.DataFrame:
    numeric_cols = df.select_dtypes(include=[np.number]).columns.tolist()
    if "round" in numeric_cols:
        numeric_cols.remove("round")
    agg = {c: "mean" for c in numeric_cols}
    non_numeric = [c for c in df.columns if c not in numeric_cols]
    for c in non_numeric:
        agg[c] = "first"
    return df.groupby("round", dropna=False, as_index=False).agg(agg)


def main() -> None:
    p = argparse.ArgumentParser()
    p.add_argument("--csv", type=str, default="finetune_main.csv")
    p.add_argument("--out", type=str, default="finetune_synthetic.csv")
    p.add_argument("--seed", type=int, default=0)
    p.add_argument("--noise-scale", type=float, default=0.01)
    args = p.parse_args()

    df = pd.read_csv(args.csv)

    required = ["device_id", "poisoning_type", "poisoning_rate", "round"]
    missing = [c for c in required if c not in df.columns]
    if missing:
        raise ValueError(f"Missing required columns: {missing}")

    dev_a = "113"
    dev_b = "114"
    a_df = df[df["device_id"].astype(str) == dev_a]
    b_df = df[df["device_id"].astype(str) == dev_b]
    if a_df.empty or b_df.empty:
        raise ValueError("Missing source devices 113 or 114 in input CSV")

    numeric_cols = df.select_dtypes(include=[np.number]).columns.tolist()
    for col in ["device_id", "poisoning_rate", "round"]:
        if col in numeric_cols:
            numeric_cols.remove(col)

    out_rows = []
    rng = np.random.default_rng(args.seed)

    # build synthetic rows per poisoning_type + poisoning_rate
    for (ptype, rate), sub_a in a_df.groupby(["poisoning_type", "poisoning_rate"], dropna=False):
        sub_b = b_df[(b_df["poisoning_type"] == ptype) & (b_df["poisoning_rate"] == rate)]
        if sub_b.empty:
            continue

        a_round = _per_device_round(sub_a)
        b_round = _per_device_round(sub_b)

        merged = pd.merge(a_round, b_round, on="round", suffixes=("_a", "_b"))
        if merged.empty:
            continue

        pooled = pd.concat([sub_a, sub_b], ignore_index=True)
        col_std = pooled[numeric_cols].std(ddof=0).replace(0, 0.0)

        for _, row in merged.iterrows():
            out = {}
            for col in df.columns:
                if col in numeric_cols:
                    val = 0.5 * (row[f"{col}_a"] + row[f"{col}_b"])
                    noise = rng.normal(0.0, args.noise_scale * col_std.get(col, 0.0))
                    out[col] = float(val + noise)
                elif col == "device_id":
                    out[col] = "112"
                elif col == "poisoning_type":
                    out[col] = ptype
                elif col == "poisoning_rate":
                    out[col] = rate
                elif col == "round":
                    out[col] = row["round"]
                else:
                    out[col] = row.get(f"{col}_a", row.get(col, None))
            out_rows.append(out)

    synth = pd.DataFrame(out_rows)
    combined = pd.concat([df, synth], ignore_index=True)
    combined.to_csv(args.out, index=False)
    print(f"Saved synthetic CSV to: {args.out}")


if __name__ == "__main__":
    main()
