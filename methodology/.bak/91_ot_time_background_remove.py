...
# --- Time-aware sample extraction ---
def _to_samples_with_time(df: pd.DataFrame, time_mode: str = "relative") -> tuple[np.ndarray, np.ndarray]:
    """Return (X, t) where X is Nx2 [cpu, mem] and t is N timestamps aligned to X.

    Uses ts_unix if available; otherwise returns X with a synthetic index time.

    time_mode:
      - relative: t := ts_unix - min(ts_unix) within the provided df
      - absolute: t := ts_unix
    """
    if df.empty:
    ...
    # Time
    if "ts_unix" in work.columns:
        work["_t"] = pd.to_numeric(work["ts_unix"], errors="coerce")
        work = work.loc[work[cpu_col].notna() & work[mem_col].notna() & work["_t"].notna()].copy()
        if work.empty:
            return np.empty((0, 2), dtype=float), np.empty((0,), dtype=float)
        ...
        # No timestamps; use a synthetic index as time
        work = work.loc[work[cpu_col].notna() & work[mem_col].notna()].copy()
        if work.empty:
            return np.empty((0, 2), dtype=float), np.empty((0,), dtype=float)
        t = np.arange(work.shape[0], dtype=float)

    X = work[[cpu_col, mem_col]].to_numpy(dtype=float)
    return X, t
...
def _sinkhorn_plan(
    ...
    a = np.ones(n) / n
    b = np.ones(m) / m
    # squared Euclidean cost in feature space
    C = np.sum((X[:, None, :] - Y[None, :, :]) ** 2, axis=2)

    # Optional time-augmented term to discourage matching far-apart timestamps
    if time_alpha and time_alpha > 0 and tx is not None and ty is not None and tx.size == X.shape[0] and ty.size == Y.shape[0]:
        denom = float(time_scale) if time_scale and time_scale > 0 else 60.0
        dt = (tx[:, None] - ty[None, :]) / denom
        C = C + float(time_alpha) * (dt ** 2)
...
def _compute_increment_reference(
    ...
    """Return OT-implied increment distribution mapping background->training.

    If time_alpha>0 and time arrays are provided, uses time-augmented OT cost.
    """
    if Bz.size == 0 or Tz.size == 0:
        return np.empty((0, 2), dtype=float)
    P = _sinkhorn_plan(Bz, Tz, reg=reg, iters=iters, tx=Bt, ty=Tt, time_alpha=time_alpha, time_scale=time_scale)
...
def main() -> None:
    ...
    # --- Time-augmented OT options ---
    p.add_argument("--time-alpha", type=float, default=0.0, help="weight for time term in OT cost (0 disables time)")
    p.add_argument("--time-scale", type=float, default=60.0, help="seconds used to normalize time differences in OT cost")
    p.add_argument("--time-mode", type=str, default="relative", choices=["relative", "absolute"], help="use relative time (t - min(t)) per split, or absolute ts_unix")
    p.add_argument("--placebo-time", action="store_true", help="use time-based placebo split (first half vs second half by time)")
    ...
    logger = logging.getLogger("ot_bg_remove")
    if args.debug:
        logger.debug("Time-augmented OT: enabled=%s alpha=%.6g scale=%.6g mode=%s", bool(args.time_alpha and args.time_alpha > 0), float(args.time_alpha), float(args.time_scale), args.time_mode)
    ...
    for device_id, g in bg_df.groupby("device_id", dropna=False):
        if args.time_alpha and args.time_alpha > 0:
            B, Bt = _to_samples_with_time(g, time_mode=args.time_mode)
        else:
            B = _to_samples(g)
            Bt = None
        if args.debug:
        ...
        bg_by_device[str(device_id)] = {"mu": mu, "sigma": sigma, "Bz": Bz, "Bt": Bt}
    ...
    for device_id, g in tr_df.groupby("device_id", dropna=False):
        ...
        Bt = bg_by_device[dev].get("Bt", None)  # type: ignore[assignment]
        ...
        if args.time_alpha and args.time_alpha > 0:
            Tc_raw, Tct = _to_samples_with_time(g_clean, time_mode=args.time_mode)
            Tp_raw, Tpt = _to_samples_with_time(g_poison, time_mode=args.time_mode)
        else:
            Tc_raw, Tct = _to_samples(g_clean), None
            Tp_raw, Tpt = _to_samples(g_poison), None

        Tc = _standardize(Tc_raw, mu, sigma)
        Tp = _standardize(Tp_raw, mu, sigma)
        if args.debug:
        ...
        Ec = _compute_increment_reference(Bz, Tc, reg=args.reg, iters=args.iters, Bt=Bt, Tt=Tct, time_alpha=args.time_alpha, time_scale=args.time_scale)
        Ep = _compute_increment_reference(Bz, Tp, reg=args.reg, iters=args.iters, Bt=Bt, Tt=Tpt, time_alpha=args.time_alpha, time_scale=args.time_scale)
        if args.debug:
    ...
    for (device_id, poison_frac, epoch), group in mix_df.groupby(
        ...
        if args.time_alpha and args.time_alpha > 0:
            M_raw, Mt = _to_samples_with_time(group, time_mode=args.time_mode)
        else:
            M_raw, Mt = _to_samples(group), None

        M = _standardize(M_raw, mu, sigma)
        if args.debug:
        ...
        P = _sinkhorn_plan(Bz, M, reg=args.reg, iters=args.iters, tx=Bt, ty=Mt, time_alpha=args.time_alpha, time_scale=args.time_scale)
        ...
        if args.placebo_time and Bt is not None and isinstance(Bt, np.ndarray) and Bt.size == Bz.shape[0]:
            order = np.argsort(Bt)
            mid = len(order) // 2
            idx1 = order[:mid]
            idx2 = order[mid:]
        ...
        if B1.size and B2.size:
            Pbb = _sinkhorn_plan(B1, B2, reg=args.reg, iters=args.iters, tx=t1, ty=t2, time_alpha=args.time_alpha, time_scale=args.time_scale)
