...
def _mannwhitney_per_device(df: pd.DataFrame) -> pd.DataFrame:
    rows: list[dict[str, object]] = []
    for device_id, grp in df.groupby("device_id", dropna=False):
        clean = grp[grp["poisoning_type"] == "clean"]
        poisoned = grp[grp["poisoning_type"] != "clean"]
        for metric in ["ot_distance", "cosine_similarity"]:
            a = clean[metric].astype(float).dropna().to_numpy()
            b = poisoned[metric].astype(float).dropna().to_numpy()
    ...
    return pd.DataFrame(rows).sort_values(["device_id", "metric"])
...
def main() -> None:
    ...
    result = _mannwhitney_per_device(df)
    ...
    # Fisher's method across devices (exclude specified device_id)
    exclude = {s.strip() for s in args.exclude_device.split(",") if s.strip()}
    fisher_rows: list[dict[str, object]] = []
    for metric, grp in result.groupby("metric", dropna=False):
