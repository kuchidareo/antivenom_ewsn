...
def _compute_score(df: pd.DataFrame, w_ot: float, w_cos: float) -> pd.Series:
    # Higher score => more likely poisoned
    return w_ot * df["ot_distance"] + w_cos * (1.0 - df["cosine_similarity"])
...
def main() -> None:
    ...
    p.add_argument("--window", type=int, default=5, help="number of rounds to aggregate")
    args = p.parse_args()
    ...
    for device_id, grp in df.groupby("device_id", dropna=False):
        ...
        grp["window_id"] = (grp["round"].astype(int) // int(args.window))
        agg = (
            grp.groupby(
                ["device_id", "poisoning_type", "poisoning_rate", "window_id"],
                dropna=False,
            )
            .agg(
                label=("label", "max"),
    ...
    result = pd.DataFrame(rows).sort_values(["device_id"])
