...
def build_rolling_mad_points(summary: pd.DataFrame, rolling_window: int) -> pd.DataFrame:
    df = summary.copy()
    ...
    df["abs_ot_diff"] = df.groupby(run_cols, dropna=False)["ot_distance"].diff().abs()
    df["score"] = (
        df.groupby(run_cols, dropna=False)["abs_ot_diff"]
        .rolling(rolling_window, min_periods=rolling_window)
        .mean()
        .reset_index(level=run_cols, drop=True)
    )
    df["rolling_window"] = rolling_window
    df["window_end_round"] = df["round"]
    group_cols = [
        "scenario",
        "cost_function",
        "signal_mix",
    ...
    return df.dropna(subset=["score"])[
        ...
        ["classification_label", "target_group", "target_label", "window_end_round"]
    )
...
def threshold_sweep(points: pd.DataFrame) -> pd.DataFrame:
    ...
    for threshold in sorted(points["score"].dropna().unique()):
        for rule, pred in (
            (
                "poisoned_if_score_gte_threshold",
                ["poisoned" if score >= threshold else "clean" for score in points["score"]],
            ),
            (
                "poisoned_if_score_lte_threshold",
                ["poisoned" if score <= threshold else "clean" for score in points["score"]],
            ),
        ):
            y_pred = pd.Series(pred, index=points.index)
            row = {
    ...
    return pd.DataFrame(rows).sort_values(
        ["j", "f1", "recall", "precision", "rule", "threshold"],
...
def build_predictions(points: pd.DataFrame, classification: pd.DataFrame) -> pd.DataFrame:
    ...
    rule = str(classification["rule"].iloc[0])
    out["threshold"] = threshold
    out["rule"] = rule
    if rule == "poisoned_if_score_lte_threshold":
        out["predicted_label"] = [
...
def plot_rolling_threshold(
    ...
    rolling_window = int(classification["rolling_window"].iloc[0])
    threshold = float(classification["threshold"].iloc[0])
    rule = str(classification["rule"].iloc[0])
    ...
    ax.set_xlabel(f"rolling mean abs OT diff, w={rolling_window}")
    ax.set_ylabel("true class")
    ax.set_title(f"{scenario}: {signal_mix}, rolling w={rolling_window}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="c11-only rolling mean absolute OT-difference sequential classifier."
    )
    ...
    parser.add_argument("--rolling-windows", default=",".join(str(w) for w in ROLLING_WINDOWS))
    ...
    all_predictions = []
    manifest_rows = []
    rolling_windows = [int(value) for value in _parse_csv_list(args.rolling_windows)]
    ...
    for scenario in _selected_scenarios(args.scenarios):
        ...
        for mix in SIGNAL_MIXES:
            ...
            for rolling_window in rolling_windows:
                ...
                sweep.insert(0, "rolling_window", rolling_window)
                ...
                name = _safe_name(f"{scenario.name}__{COST_FUNCTION}__{mix.name}__w{rolling_window}")
                summary_path = out_dir / f"{name}_summary.csv"
                points_path = out_dir / f"{name}_rolling_mad_points.csv"
                ...
                plot_path = out_dir / "plots" / f"{name}_rolling_mad.png"
                ...
                plot_rolling_threshold(points, classification, plot_path)
                ...
                print(f"Saved {scenario.name} {mix.name} w={rolling_window}: {classification_path}")
    ...
    if all_points:
        pd.concat(all_points, ignore_index=True).to_csv(
            out_dir / "all_scenarios_c11_rolling_mad_points.csv",
            index=False,
        )
    if all_sweeps:
