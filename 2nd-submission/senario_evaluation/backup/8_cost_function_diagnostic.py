...
def _plot_diagnostics(
    ...
    for row_idx, (signal_name, signal_label) in enumerate(SIGNAL_ORDER):
        ...
        tau_centers, val_means, bin_counts = _binned_mean_curve(time_diff, value_diff, bins=relation_bins)
...
def main() -> None:
    ...
    p.add_argument("--out-dir", default=str(ROOT_DIR / "8_cost_function_diagnostics"))
    ...
    p.add_argument(
        ...
        type=int,
        default=24,
        help="number of bins for g(tau) over relative time",
    )
    p.add_argument(
        "--scatter-max-points",
        type=int,
        default=12000,
        help="maximum number of (time gap, value gap) points to draw in the scatter",
    )
    p.add_argument(
        "--reg-scale",
        type=float,
        default=0.05,
        help="reg scale used to report the current adaptive Sinkhorn regularization from the cost matrix",
    )
    p.add_argument(
        "--sharpness-eps",
        type=float,
        default=1e-9,
        help="epsilon used in R_i = d_(2) / (d_(1) + eps)",
    )
    args = p.parse_args()
