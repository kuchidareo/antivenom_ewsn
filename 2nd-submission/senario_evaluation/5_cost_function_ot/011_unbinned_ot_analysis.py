...
DEFAULT_COST_FUNCTION = "c4_time_value"
STEP1_COST_FUNCTIONS = (
    "c1_value",
    "c2_delta",
    "c3_time",
    "c4_time_value",
...
def _curvature_features(measure: dict[str, Any]) -> np.ndarray:
    cache = measure.setdefault("_curvature_cache", None)
    if cache is None:
        x = _ensure_2d(measure["x"])
        ...
        measure["_curvature_cache"] = curvature
        cache = curvature
    return np.asarray(cache, dtype=float)


def _zscore_value_features(measure: dict[str, Any]) -> np.ndarray:
    cache = measure.setdefault("_zscore_value_cache", None)
    if cache is None:
        x = _ensure_2d(measure["x"])
        if x.size == 0:
            z = x.copy()
            ...
            std = np.where(std > 1e-8, std, 1.0)
            z = (x - mean) / std
        measure["_zscore_value_cache"] = z
        cache = z
    return np.asarray(cache, dtype=float)


def _local_variance_features(
    measure: dict[str, Any],
    window_size: int,
    pad_mode: str,
) -> np.ndarray:
    cache = measure.setdefault("_local_variance_cache", {})
    cache_key = (int(window_size), str(pad_mode))
    ...
    n_points, n_dim = x.shape
    if n_points == 0:
        features = np.zeros((0, n_dim), dtype=float)
    else:
        window_size = max(int(window_size), 0)
        window_width = 2 * window_size + 1
        if window_size == 0:
            ...
            padded = np.pad(x, ((window_size, window_size), (0, 0)), mode=pad_mode)
            windows = np.stack([padded[idx : idx + window_width, :] for idx in range(n_points)], axis=0)
        features = np.var(windows, axis=1)
...
def _frequency_features(
    ...
    cache = measure.setdefault("_frequency_cache", {})
    cache_key = (int(window_size), bool(z_normalize_window), str(pad_mode))
    if cache_key in cache:
        return np.asarray(cache[cache_key], dtype=float)

    windows = _local_window_features(
        measure["x"],
        window_size=int(window_size),
        z_normalize_window=bool(z_normalize_window),
        pad_mode=str(pad_mode),
    ...
    if windows.size == 0:
        features = np.zeros((0, 1), dtype=float)
    else:
        x = _ensure_2d(measure["x"])
        n_dim = x.shape[1]
        window_width = 2 * max(int(window_size), 0) + 1
        reshaped = windows.reshape(len(windows), window_width, n_dim)
        fft_mag = np.abs(np.fft.rfft(reshaped, axis=1))
        if fft_mag.shape[1] > 1:
            fft_mag = fft_mag[:, 1:, :]
        features = fft_mag.reshape(len(windows), -1)
...
def _prepare_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Epoch 9 is an unstable outlier in these 120-second logs, so keep
    # the same 0..8 analysis window used by the earlier visualizations.
...
def _cost_matrix(
    ...
    aliases = {
        ...
        "delta": "c2_delta",
        "time": "c3_time",
        "time_value": "c4_time_value",
        "frequency": "c5_frequency",
        "curvature": "c6_curvature",
        "window": "c7_window",
        "shape_zscore": "c8_shape_zscore",
        "local_variance": "c9_local_variance",
    ...
    if cost_function == "c5_frequency":
        ...
        target_freq = _frequency_features(
            target,
            window_size=int(window_size),
            z_normalize_window=bool(z_normalize_window),
            pad_mode=str(pad_mode),
        )
        frequency_cost = float(beta) * _pairwise_squared_l2(ref_freq, target_freq)
    else:
        frequency_cost = np.zeros((len(ref["t"]), len(target["t"])), dtype=float)

    if cost_function == "c6_curvature":
        ref_curvature = _curvature_features(ref)
        target_curvature = _curvature_features(target)
        curvature_cost = float(beta) * _pairwise_squared_l2(ref_curvature, target_curvature)
    else:
    ...
    if cost_function == "c7_window":
        ...
        target_window = _shape_features(
            target,
            window_size=int(window_size),
            z_normalize_window=False,
            pad_mode=str(pad_mode),
        )
        window_cost = float(beta) * _pairwise_squared_l2(ref_window, target_window)
    else:
        window_cost = np.zeros((len(ref["t"]), len(target["t"])), dtype=float)

    if cost_function == "c8_shape_zscore":
        ref_z = _zscore_value_features(ref)
        target_z = _zscore_value_features(target)
        zscore_value_cost = float(beta) * _pairwise_squared_l2(ref_z, target_z)
    else:
        zscore_value_cost = np.zeros((len(ref["t"]), len(target["t"])), dtype=float)

    if cost_function == "c9_local_variance":
        ref_var = _local_variance_features(
            ref,
            window_size=int(window_size),
            pad_mode=str(pad_mode),
        )
        target_var = _local_variance_features(
            target,
            window_size=int(window_size),
            pad_mode=str(pad_mode),
        )
        local_variance_cost = float(beta) * _pairwise_squared_l2(ref_var, target_var)
    else:
        local_variance_cost = np.zeros((len(ref["t"]), len(target["t"])), dtype=float)

    if cost_function == "c1_value":
        total_cost = value_cost
    elif cost_function == "c2_delta":
        total_cost = delta_cost
    elif cost_function == "c3_time":
        total_cost = time_cost
    elif cost_function == "c4_time_value":
        total_cost = squared_time_cost + value_cost
    elif cost_function == "c5_frequency":
        total_cost = frequency_cost
    elif cost_function == "c6_curvature":
        total_cost = curvature_cost
    elif cost_function == "c7_window":
        total_cost = window_cost
    elif cost_function == "c8_shape_zscore":
    ...
    if return_terms:
        return total_cost, {
            "time_cost": time_cost,
            "squared_time_cost": squared_time_cost,
            ...
            "zscore_value_cost": zscore_value_cost,
            "local_variance_cost": local_variance_cost,
...
def _transport_plan(
    ...
    if len(ref["t"]) == 0 or len(target["t"]) == 0:
        ...
        details = {
            ...
            "frequency_cost_mean": float("nan"),
            "curvature_cost_mean": float("nan"),
            "window_cost_mean": float("nan"),
            "zscore_value_cost_mean": float("nan"),
            "local_variance_cost_mean": float("nan"),
    ...
    details = {
        ...
        "frequency_cost_mean": float(np.sum(plan * terms["frequency_cost"])) if plan.size else float("nan"),
        "curvature_cost_mean": float(np.sum(plan * terms["curvature_cost"])) if plan.size else float("nan"),
        "window_cost_mean": float(np.sum(plan * terms["window_cost"])) if plan.size else float("nan"),
        "zscore_value_cost_mean": float(np.sum(plan * terms["zscore_value_cost"])) if plan.size else float("nan"),
        "local_variance_cost_mean": float(np.sum(plan * terms["local_variance_cost"])) if plan.size else float("nan"),
        "shape_cost_enabled": 0.0,
        "cost_function": str(terms["cost_function"]),
    }
...
def build_summary(
    ...
    template_signatures: dict[str, np.ndarray] = {}
    for signal_name in ["mem", *CORE_COLS]:
        template_measure = _aggregate_run_measure(
            [
                per_run_measures[template_run_key][signal_name][epoch]
                for epoch in sorted(per_run_measures[template_run_key][signal_name])
            ]
        )
        template_plan, _, _ = _transport_plan(
            ref_measures[signal_name],
            template_measure,
            reg_scale=reg_scale,
            alpha=alpha,
            beta=beta,
        ...
        template_signatures[signal_name] = _plan_signature(template_plan)
    ...
    for _, row in summary_per_epoch.iterrows():
        ...
        core_frequency_cost_means: list[float] = []
        core_curvature_cost_means: list[float] = []
        core_window_cost_means: list[float] = []
        core_zscore_value_cost_means: list[float] = []
        core_local_variance_cost_means: list[float] = []
        ...
        for core_idx, col in enumerate(CORE_COLS):
            ...
            core_frequency_cost_means.append(detail["frequency_cost_mean"])
            core_curvature_cost_means.append(detail["curvature_cost_mean"])
            core_window_cost_means.append(detail["window_cost_mean"])
            core_zscore_value_cost_means.append(detail["zscore_value_cost_mean"])
            core_local_variance_cost_means.append(detail["local_variance_cost_mean"])
            core_similarities.append(_cosine_similarity(_plan_signature(plan), template_signatures[col]))
        ...
        mem_similarity = _cosine_similarity(_plan_signature(mem_plan), template_signatures["mem"])
        core_similarity_mean = _nanmean(core_similarities)
        ...
        rows.append(
            {
                ...
                "mem_frequency_cost_mean_to_baseclean": mem_detail["frequency_cost_mean"],
                "mem_curvature_cost_mean_to_baseclean": mem_detail["curvature_cost_mean"],
                "mem_window_cost_mean_to_baseclean": mem_detail["window_cost_mean"],
                "mem_zscore_value_cost_mean_to_baseclean": mem_detail["zscore_value_cost_mean"],
                "mem_local_variance_cost_mean_to_baseclean": mem_detail["local_variance_cost_mean"],
                ...
                "core0_frequency_cost_mean_to_baseclean": signal_term_details["core_0"]["frequency_cost_mean"],
                "core1_frequency_cost_mean_to_baseclean": signal_term_details["core_1"]["frequency_cost_mean"],
                "core2_frequency_cost_mean_to_baseclean": signal_term_details["core_2"]["frequency_cost_mean"],
                "core3_frequency_cost_mean_to_baseclean": signal_term_details["core_3"]["frequency_cost_mean"],
                "core0_curvature_cost_mean_to_baseclean": signal_term_details["core_0"]["curvature_cost_mean"],
                "core1_curvature_cost_mean_to_baseclean": signal_term_details["core_1"]["curvature_cost_mean"],
                "core2_curvature_cost_mean_to_baseclean": signal_term_details["core_2"]["curvature_cost_mean"],
                "core3_curvature_cost_mean_to_baseclean": signal_term_details["core_3"]["curvature_cost_mean"],
                "core0_window_cost_mean_to_baseclean": signal_term_details["core_0"]["window_cost_mean"],
                "core1_window_cost_mean_to_baseclean": signal_term_details["core_1"]["window_cost_mean"],
                "core2_window_cost_mean_to_baseclean": signal_term_details["core_2"]["window_cost_mean"],
                "core3_window_cost_mean_to_baseclean": signal_term_details["core_3"]["window_cost_mean"],
                "core0_zscore_value_cost_mean_to_baseclean": signal_term_details["core_0"]["zscore_value_cost_mean"],
                "core1_zscore_value_cost_mean_to_baseclean": signal_term_details["core_1"]["zscore_value_cost_mean"],
                "core2_zscore_value_cost_mean_to_baseclean": signal_term_details["core_2"]["zscore_value_cost_mean"],
                "core3_zscore_value_cost_mean_to_baseclean": signal_term_details["core_3"]["zscore_value_cost_mean"],
                "core0_local_variance_cost_mean_to_baseclean": signal_term_details["core_0"]["local_variance_cost_mean"],
                "core1_local_variance_cost_mean_to_baseclean": signal_term_details["core_1"]["local_variance_cost_mean"],
                "core2_local_variance_cost_mean_to_baseclean": signal_term_details["core_2"]["local_variance_cost_mean"],
                "core3_local_variance_cost_mean_to_baseclean": signal_term_details["core_3"]["local_variance_cost_mean"],
                "mem_similarity_to_template": mem_similarity,
                "core0_similarity_to_template": core_similarities[0],
                "core1_similarity_to_template": core_similarities[1],
                "core2_similarity_to_template": core_similarities[2],
                "core3_similarity_to_template": core_similarities[3],
                ...
                "core_frequency_cost_mean_to_baseclean": float(np.mean(core_frequency_cost_means)),
                "core_curvature_cost_mean_to_baseclean": float(np.mean(core_curvature_cost_means)),
                "core_window_cost_mean_to_baseclean": float(np.mean(core_window_cost_means)),
                "core_zscore_value_cost_mean_to_baseclean": float(np.mean(core_zscore_value_cost_means)),
                "core_local_variance_cost_mean_to_baseclean": float(np.mean(core_local_variance_cost_means)),
                "core_similarity_mean_to_template": core_similarity_mean,
                "ot_distance": 0.8 * float(np.mean(dists)) + 0.2 * mem_dist,
                "similarity_score": 0.8 * core_similarity_mean + 0.2 * mem_similarity,
                ...
                "cost_function": str(mem_detail["cost_function"]),
...
def main() -> None:
    ...
    p.add_argument(
        ...
        help="select the OT cost: c1 value through c9 local variance",
    )
    p.add_argument(
        "--use-shape-cost",
        dest="use_shape_cost",
