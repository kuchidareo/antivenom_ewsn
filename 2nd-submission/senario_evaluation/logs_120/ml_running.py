...
def _apply_weight_norm(module: nn.Module) -> nn.Module:
    for name, child in module.named_children():
        if isinstance(child, (nn.Conv2d, nn.Linear)):
            setattr(module, name, weight_norm(child, name="weight", dim=0))
        else:
            _apply_weight_norm(child)
    return module
...
class SimpleCNN(nn.Module):
    def __init__(self, n_classes: int, img_size: int) -> None:
        ...
        # 3 conv layers
        self.conv1 = weight_norm(nn.Conv2d(3, 32, kernel_size=3, padding=1), name="weight", dim=0)
        self.conv2 = weight_norm(nn.Conv2d(32, 64, kernel_size=3, padding=1), name="weight", dim=0)
        self.conv3 = weight_norm(nn.Conv2d(64, 128, kernel_size=3, padding=1), name="weight", dim=0)
        ...
        # 3 fc layers
        self.fc1 = weight_norm(nn.Linear(feat_dim, 256), name="weight", dim=0)
        self.fc2 = weight_norm(nn.Linear(256, 128), name="weight", dim=0)
        self.fc3 = weight_norm(nn.Linear(128, n_classes), name="weight", dim=0)
