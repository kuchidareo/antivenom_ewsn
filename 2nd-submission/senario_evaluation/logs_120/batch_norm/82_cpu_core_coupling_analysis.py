...
def cpu_core_sorted_ot(df: pd.DataFrame, bins: int = 50) -> pd.DataFrame:
    ...
    for key, group in df.groupby(
        ["device_id", "source_label", "poisoning_type", "poison_frac", "run_csv", "epoch"],
        dropna=False,
    ):
        group_key = key[:5]
        per_epoch_shapes.setdefault(group_key, {c: [] for c in core_cols})
        for col in core_cols:
            shape = _epoch_shape_mean(group, col, bins=bins)
            per_epoch_shapes[group_key][col].append(shape)

    rows: list[dict[str, object]] = []
    for (device_id, source_label, poisoning_type, poison_frac, run_csv), shapes in per_epoch_shapes.items():
        mean_shapes = {}
        for col in core_cols:
    ...
    ref_rows = summary[summary["source_label"] == "baseclean"]
    ref_by_device = {
        row["device_id"]: {
    ...
    clean_rows = summary[summary["source_label"] == "clean"]
    clean_delta_by_device: dict[object, dict[int, np.ndarray]] = {}
    for _, row in clean_rows.iterrows():
        ...
        if ref is None or device_id not in ref_coupling_by_device:
            continue
        clean_delta_by_device[device_id] = {}
        for idx in range(4):
            clean_shape = np.array(json.loads(row[f"core{idx}_shape_mean"]), dtype=float)
            ref_shape = np.array(ref[f"core_{idx}"], dtype=float)
            gs_ref = ref_coupling_by_device[device_id][idx]
            gs_clean = _ot_coupling_1d(ref_shape, clean_shape)
            clean_delta_by_device[device_id][idx] = (gs_clean - gs_ref).ravel()
    ...
    for _, row in summary.iterrows():
        ...
        for idx in range(4):
            ...
            tmpl = clean_delta_by_device.get(device_id, {}).get(idx)
        ...
        distances.append(
            {
                ...
                "core0_type_cosine_to_clean": cos_sims[0] if len(cos_sims) > 0 else float("nan"),
                "core1_type_cosine_to_clean": cos_sims[1] if len(cos_sims) > 1 else float("nan"),
                "core2_type_cosine_to_clean": cos_sims[2] if len(cos_sims) > 2 else float("nan"),
                "core3_type_cosine_to_clean": cos_sims[3] if len(cos_sims) > 3 else float("nan"),
                "core_type_cosine_mean_to_clean": cos_mean,
                "core_ot_distance_mean": float(np.mean(dists)),
                "core_ot_distance_mean_to_baseclean": float(np.mean(dists)),
            }
        )

    dist_df = pd.DataFrame(distances).sort_values(["device_id", "target_label"])
    return dist_df
...
if __name__ == "__main__":
    ...
    cols = [
        ...
        "poisoning_type",
        "poison_frac",
        "core0_type_cosine_to_clean",
        "core1_type_cosine_to_clean",
        "core2_type_cosine_to_clean",
        "core3_type_cosine_to_clean",
        "core_type_cosine_mean_to_clean",
        "core_ot_distance_mean_to_baseclean",
    ]
