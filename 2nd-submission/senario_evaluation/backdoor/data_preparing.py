...
Each JSONL line contains at least:
  {"file": "<filename>.jpg", "label": <int>, "split": "train|test"}

Backdoor metadata also includes `orig_label`, `target_label`, `is_backdoor`,
and trigger details so evaluation can separate clean accuracy from attacked
sample behavior.

Example:
  python data_preparing.py --dataset kuchidareo/small_trashnet
"""
...
class PreprocessConfig:
    ...
    # BadNets backdoor: visible square trigger in the bottom-right corner.
    backdoor_trigger_frac: float = 0.10
    backdoor_trigger_value: int = 255
...
def resize_for_output(img: Image.Image, cfg: PreprocessConfig) -> Image.Image:
    if cfg.img_size <= 0:
        return img
    return img.resize((cfg.img_size, cfg.img_size), resample=Image.BILINEAR)


def next_label(label: int, n_classes: int) -> int:
    if n_classes <= 0:
        return label
    return (int(label) + 1) % n_classes


def badnets_trigger_size(img: Image.Image, cfg: PreprocessConfig) -> tuple[int, int]:
    w, h = img.size
    frac = max(0.0, min(1.0, float(cfg.backdoor_trigger_frac)))
    trigger_w = max(1, min(w, int(math.ceil(w * frac))))
    trigger_h = max(1, min(h, int(math.ceil(h * frac))))
    return trigger_w, trigger_h


def apply_badnets_trigger(img: Image.Image, cfg: PreprocessConfig) -> Image.Image:
    img = ensure_rgb(img)
    arr = np.array(img).copy()
    trigger_w, trigger_h = badnets_trigger_size(img, cfg)
    value = max(0, min(255, int(cfg.backdoor_trigger_value)))
    arr[-trigger_h:, -trigger_w:, :] = value
...
def process_split(
    ...
    for pos, idx in enumerate(tqdm(source_indices, desc=f"Processing {split_name}")):
        ...
        img = resize_for_output(ensure_rgb(img), cfg)
        ...
        if "backdoor" in selected_variants:
            # BadNets: bottom-right trigger and deterministic +1 relabeling.
            v = "backdoor"
            file_id = stable_id(split_name, idx, v)
            filename = f"{split_name}_{file_id}.jpg"
            out_path = out_root / v / filename
            backdoor_applied = pos in variant_poisoned_positions[v]
            target_label = next_label(label, n_classes=n_classes)
            new_label = target_label if backdoor_applied else label
            new_label_name = class_names[new_label] if 0 <= new_label < n_classes else str(new_label)
            target_label_name = (
                class_names[target_label] if 0 <= target_label < n_classes else str(target_label)
            )
            out_img = apply_badnets_trigger(img, cfg=cfg) if backdoor_applied else img
            trigger_w, trigger_h = badnets_trigger_size(img, cfg)
            save_jpeg(out_img, out_path)
            append_jsonl(
                out_root / v / "metadata.jsonl",
                {
                    "file": filename,
                    "label": new_label,
                    "split": split_name,
                    "variant": v,
                    "poison_type": v,
                    "poison_applied": backdoor_applied,
                    "is_backdoor": backdoor_applied,
                    ...
                    "label_changed": backdoor_applied and new_label != label,
                    "trigger_position": "bottom-right",
                    "trigger_frac": cfg.backdoor_trigger_frac,
                    "trigger_width": trigger_w,
                    "trigger_height": trigger_h,
                    "trigger_value": cfg.backdoor_trigger_value,
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--out",
        type=str,
        default="/home/user/kuchida/antivenom_ewsn/data/backdoor",
    ...
    p.add_argument(
        "--backdoor-trigger-frac",
        type=float,
        default=0.10,
        help="BadNets trigger width/height as a fraction of --img-size.",
    )
    p.add_argument(
        "--backdoor-trigger-value",
        type=int,
        default=255,
        help="RGB value for the BadNets square trigger.",
    )
    p.add_argument(
        "--ood-frac",
        type=float,
...
def main() -> None:
    ...
    cfg = PreprocessConfig(
        ...
        backdoor_trigger_frac=max(0.0, min(1.0, float(args.backdoor_trigger_frac))),
        backdoor_trigger_value=max(0, min(255, int(args.backdoor_trigger_value))),
