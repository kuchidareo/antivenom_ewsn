...
DEFAULT_CASES = (
    "weight_normalization/clean",
    "weight_normalization/blurring",
    "backward_stabilization/clean",
    "backward_stabilization/blurring",
)
EXCLUDED_RUNS = {
...
def _viz_module():
    ...
    return _load_module("scenario_best_match_alignment_viz_mod", ROOT_DIR / "91_best_match_time_alignment_visualization.py")
...
def main() -> None:
    ...
    p.add_argument("--out-dir", default=str(ROOT_DIR / "results"))
    p.add_argument("--min-runs", type=int, default=2)
    p.add_argument(
        "--case",
        action="append",
        default=[],
        help="case to include, such as weight_normalization/clean; repeatable. Defaults to the two requested scenarios.",
    )
    p.add_argument(
    ...
    requested_cases = tuple(args.case) if args.case else DEFAULT_CASES
    missing_cases = [case_name for case_name in requested_cases if case_name not in cases]
    if missing_cases:
        raise ValueError(f"Requested cases were not found under {root}: {missing_cases}")
    base_cases = [case_name for case_name in base_cases if case_name in requested_cases]
    if not base_cases:
        raise ValueError(f"No eligible requested cases found in {root}. Requested: {requested_cases}")
    ...
    for base_case in base_cases:
        ...
        if args.mode in {"template", "both"}:
            template_png = base_out_dir / "00_reference_vs_template_best_match_alignment.png"
            summary = viz._plot_best_match_alignment(
                out_path=template_png,
                pair_title=f"Best Match Alignment: {base_case} Reference vs Template",
        ...
        if args.mode in {"target", "both"}:
            for idx, target_spec in enumerate(target_specs, start=1):
                ...
                png_path = base_out_dir / f"{idx:02d}_{_slugify(target_group)}_{target_csv.stem}_best_match_alignment.png"
                summary = viz._plot_best_match_alignment(
                    out_path=png_path,
                    pair_title=f"Best Match Alignment: {base_case} Reference vs {target_group}",
                    ref_title=reference_csv.stem,
    ...
    manifest_path = out_dir / "iterative_best_match_alignment_manifest.json"
