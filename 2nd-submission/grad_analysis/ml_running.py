...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--save-grad",
        action="store_true",
        help="Save per-step gradient snapshots as .pt files during training",
    )
    p.add_argument(
        "--grad-log-root",
        type=str,
        default="grad_logs",
        help="Root directory for saved gradient .pt files when --save-grad is enabled",
    )
    p.add_argument(
        "--disable-perf",
        action="store_true",
