...
def grad_distribution_stats(grad: torch.Tensor) -> Dict[str, float]:
    ...
    activated_values = values[values != 0]
    signed = kurtosis_stats(activated_values)
    absolute = kurtosis_stats(activated_values.abs())

    return {
        "num_values": signed["num_values"],
...
def grad_layer_energy_stats(grads: Dict[str, torch.Tensor]) -> tuple[Dict[str, Dict[str, float]], float]:
    ...
    max_energy_share = max(sqsum.values()) / total_energy if sqsum else 0.0
    stats = {
        key: {
            "l2_norm": energy ** 0.5,
            "energy": energy,
            "num_values": numel[key],
        }
...
def analyze_run(
    ...
    for pt_file in list_grad_files(grad_dir):
        ...
        layer_energy_stats, max_energy_share = grad_layer_energy_stats(grads)
        ...
        for layer, stats in sorted(layer_energy_stats.items()):
            rows.append(
                {
                    "condition": canonical_condition(condition),
                    "epoch": epoch,
